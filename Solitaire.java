...
public class Solitaire {
	...
	public void makeDeck(StringTokenizer st) throws OverListException,
			UnderListException, JokerException {// Throw the data of deck.txt to
								// the list
		...
		if (count < 29) {// if the deck is under 28
			...
			throw new JokerException(
					"Joker A Exception, the number of Joker A : " + countA);
		} else if (countB > 1 || countB == 0) {
			// if the Joker B is more than one or less than one
			throw new JokerException(
					"Joker B Exception, the number of Joker B : " + countB);
		}
	...
	public void jokerA() {
		...
		if (head.cardValue != 27) {// if head is not JokerA
			while (current2.next.cardValue != 27)
	...
	public void tripleCut() {
		...
		if (last.cardValue == 27 || last.cardValue == 28) { // if the tail is Joker A or Joker B
			...
			while (current.next != tail) {
				if (current.next.cardValue == 27
						|| current.next.cardValue == 28) {
					firstJoker = current;
					break;
				} else {
					current = current.next;
	...
	public void countCut() {
		...
		for (int i = 0; i < tail.cardValue; i++) {
			...
			current.next = new CardNode(temp.cardValue, tail); // Add to current ( The number 2 from the rear )
	...
	public void encryption(Solitaire deck, char message[]) {
		...
		for (int i = 0; i < message.length; i++) { // if user's message equals with the character of index array
	...
	public void decryption(Solitaire deck, char message[]) {
		...
		for (int i = 0; i < message.length; i++) { // User's message length
			for (int j = 0; j < index.length; j++) {
				if (message[i] == index[j]) { // if encrypted message equals with the character of index array
								// then find the character number is array
					indexNumber = j;
					break;
				}
			}
			System.out.print(message[i] + "\t" + indexNumber); // Print the message character and character number of index array
			System.out.print("\t");
			// Print the deck number 
			System.out.print(current.cardValue);
